...
describe('useChat', () => {
  ...
  describe('loadThread', () => {
    it('loads and transforms messages', async () => {
      vi.mocked(fetchThread).mockResolvedValue({
        thread: { id: 't1', title: 'Test', source: 'web', job_run_id: null, archived_at: null, created_at: '', updated_at: ''},
        messages: [
    ...
    it('extracts content from JSONB message format', async () => {
      vi.mocked(fetchThread).mockResolvedValue({
        thread: { id: 't1', title: null, source: 'web', job_run_id: null, archived_at: null, created_at: '', updated_at: ''},
        messages: [
  ...
  describe('send', () => {
    ...
    it('returns null when thread already exists', async () => {
      ...
      vi.mocked(fetchThread).mockResolvedValue({
        thread: { id: 'existing', title: null, source: 'web', job_run_id: null, archived_at: null, created_at: '', updated_at: ''},
        messages: [],
      });
