...
export const DepartureCheckConfigV1 = z.object({
    ...
    arrivalTime: z.string().regex(/^\d{2}:\d{2}$/).describe('Required arrival time in HH:MM 24-hour format (e.g. "08:30").'),
    leadMinutes: z.number().int().min(1).max(60).default(7).describe('Minutes of buffer before you need to leave. Default: 7.'),
    pollIntervalMinutes: z.number().int().min(1).max(30).default(5).describe('Minutes between traffic checks. Default: 5.'),
    notificationTitle: z.string().default('Time to leave').describe('Notification title. Default: "Time to leave".'),
    notificationBody: z.string().default('Leave now — ~{{durationMin}} min drive via {{routeSummary}}').describe('Notification body template. Supports {{durationMin}} and {{routeSummary}} placeholders.'),
    notificationTag: z.string().default('car').describe('Notification tag/emoji. Default: "car".'),
});
        ...
        while (!context.signal.aborted) {
            ...
            try {
                ...
                if (leaveBy.getTime() <= now.getTime() + leadMs) {
                    ...
                    }, 'Departure notification sent');
                    return {
                        success: true,
                        message: `Notification sent — leave by ${leaveBy.toLocaleTimeString('en-US', {
                            hour: '2-digit',
                            minute: '2-digit'
                        })}`,
                        disableJob: true,
